...
- Install missing modpacks through Prism Launcher's UI with Computer Use. Open **Add Instance**, set **Group** to **Codex**, select the provider, search the exact title, select the exact project, verify the displayed release/version, then install. Every newly installed modpack belongs in the **Codex** group.
...
- Brave opens every required CurseForge mod or resource-pack download page. Give each newly opened download tab focus once; CurseForge starts the download automatically when the page gains focus, so do not click a download button. After the downloads finish, close those mod/resource-pack tabs, return to Prism, and wait until every row is green and the footer says **All mods found**, then click **OK**. Use **try again** only for a tab whose file still did not arrive; avoid duplicate downloads.
