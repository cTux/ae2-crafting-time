...
- After **Open Missing**, activate each newly opened browser tab once so every CurseForge countdown starts. Return to Prism and wait until every row is green and the footer says **All mods found**, then click **OK**. Use **try again** only for a tab whose file still did not arrive; avoid duplicate downloads.
