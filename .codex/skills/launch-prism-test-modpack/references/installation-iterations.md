...
| 16 | CurseForge | All of Fabric 7 2.5.3 | 1.20.1 | Fabric | 95.0s | 257.3s | Installed after 2 manual files | Switch providers and wait before retyping: the preserved All of Fabric 8 query immediately exposed the compatible AOF7 predecessor. |
| 17 | Modrinth | Remarkably Optimized 1.15.46 | 1.21.1 | Fabric | 470.8s | 508.2s | Installed | Page through the Minecraft-version filter with `Page Down`; a filtered-out Sodium Plus proved incompatibility, so clear the query and choose the highest-ranked compatible pack. |
| 18 | Modrinth | OptiFabric 6.1.5 | 1.21.1 | Fabric | 140.3s | 175.4s | Installed | Reapply the compatibility filters, select a visible compatible catalog result directly, and trust the exact closed release label; no search or history picker was needed. |
| 19 | Modrinth | FPS Modpack 1.21.1 [2.6] | 1.21.1 | Fabric | 163.8s | 197.3s | Installed | Prism resets provider and compatibility filters on each new dialog even though **Group** persists; reapply them immediately instead of checking for persistence. |
| 20 | Modrinth | Fresh & Smooth 1.5.0 | 1.21.1 | Fabric | 158.4s | 192.0s | Installed | After selecting the Minecraft filter, dismiss its still-open checklist through the empty detail pane before clicking the visible compatible result. |
| 21 | CurseForge | All the Mods 10 - ATM10 8.0 | 1.21.1 | NeoForge | not measured | not measured | Installed in Codex VM; no blocked files | Use the VM's direct localhost-VNC path, and create the `Codex` group after installation when a fresh Prism profile has no groups. |
