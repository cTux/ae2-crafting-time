...
| 4 | AE2 table/UI hook | `AbstractTableRendererMixin` already covers screens that reuse AE2's table renderer. **Planned:** add a shared `AEBaseScreen` hook only if one common method can expose TTC without screen-specific assumptions. | ExtendedAE, ExtendedAE-Plus, BM Addon, Crazy AE2, Modern AE2 Additions, Applied Botanics, AE2 Import Export Card, AE2 Things, Expanded AE |
