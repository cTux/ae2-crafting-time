...
- Optional add-on CPU fixtures currently cover AdvancedAE, BM Addon, Crazy AE2
  Addons, ExtendedAE, ExtendedAE-Plus, MEGA Cells, NeoEco AE, OMNI Cells,
  ...
  cobblestone supply with a bound EMC Storage Cell. The other provider scenarios
  use an existing idle CPU because those addons do not add one.
