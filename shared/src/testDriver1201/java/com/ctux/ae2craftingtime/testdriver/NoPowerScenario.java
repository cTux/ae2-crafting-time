    ...
    boolean tick(Minecraft minecraft, FixtureMarker marker, Map<String, Boolean> checks,
        ...
        if (phase < 3) {
            if (serverStep(minecraft, player -> {
                ...
                if (ready && phase == 0) {
                    ...
                    player.serverLevel().setBlockAndUpdate(fixture.cpuPosition.east(6).north(), Blocks.AIR.defaultBlockState());
                    player.serverLevel().setBlockAndUpdate(fixture.cpuPosition.east(6).below(),
        ...
        if (phase == 3) {
            ...
            if (serverStep(minecraft, player -> {
                ...
                powerRestoredTick = player.serverLevel().getGameTime();
            ...
            if (serverStep(minecraft, player -> {
                ...
                var clear = ProfilerBridge.blockReasons(cpu.getCluster(), grid,
                        player.serverLevel().getGameTime()).get(output) == null;
