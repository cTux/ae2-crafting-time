...
public final class StatsNetwork {
    ...
    public static void sendToServer(StatsChatC2S packet) {
        if (canSend(StatsChatC2S.TYPE.id())) PacketDistributor.sendToServer(packet);
    }

    public static void sendToServer(ProviderLocateC2S packet) {
        if (canSend(ProviderLocateC2S.TYPE.id())) PacketDistributor.sendToServer(packet);
    }

    public static void sendToServer(WarningPreferenceC2S packet) {
        if (canSend(WarningPreferenceC2S.TYPE.id())) PacketDistributor.sendToServer(packet);
    }
    public static void sendToServer(ServerOptionsUpdateC2S packet) { if (canSend(ServerOptionsUpdateC2S.TYPE.id())) PacketDistributor.sendToServer(packet); }
    public static void sendToServer(StatsRequestC2S packet) { if (canSend(StatsRequestC2S.TYPE.id())) PacketDistributor.sendToServer(packet); }
    public static void sendToServer(CpuTtcRequestC2S packet) { if (canSend(CpuTtcRequestC2S.TYPE.id())) PacketDistributor.sendToServer(packet); }
    ...
    private static boolean canSend(net.minecraft.resources.ResourceLocation id) {
        var listener = Minecraft.getInstance().getConnection();
        return listener != null && NetworkRegistry.hasChannel(listener, id);
    }

    private static boolean canSend(ServerPlayer player, net.minecraft.resources.ResourceLocation id) {
