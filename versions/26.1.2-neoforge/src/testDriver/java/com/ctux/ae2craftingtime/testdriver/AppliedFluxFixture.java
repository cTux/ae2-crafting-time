...
final class AppliedFluxFixture extends AddonCpuFixture<AppliedFluxFixture.Placement> {
    ...
    protected Placement place(ServerPlayer player, FixtureMarker marker) {
        ...
        if (!(player.level().getBlockEntity(terminal) instanceof IInWorldGridNodeHost terminalHost)) {
        ...
        inventory.setItemDirect(slot, new ItemStack(AFSingletons.FE_CELL_1k.get()));
